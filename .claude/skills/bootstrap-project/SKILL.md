...
- "我有一些章节，帮我初始化项目"
- "从已有章节开始规划" / "导入全书并开始规划"
- "我有 premise 想从零开始" → **路由 `outline-tingle`**（本 Skill 是从已有成稿逆向产工件；premise 正向发散到大纲由 outline-tingle 负责，2 session 形态：premise→主题→L1→L2→L3。若用户后续需补 voice/character/world 逆向，可在大纲形成后再跑本 Skill，1a 会自动跳过）
...
> **1a 前置短路判定**（在执行 1a 大纲逆向前先做）：读 `outline.md` frontmatter `workflow_position` 与 L1-L3 实质填充度——
> - `workflow_position: outline-tingle-step2-done` → **跳过 1a**（outline-tingle 已正向产大纲），直接进 1b-1e
> - 或 L1-L3 实质填充（核心字段无 `（待定）` 占位）→ **同样跳过 1a**（兼容手填 outline 已填实的情况）
> - 否则 → 跑 1a 大纲逆向，产 `_outline-draft.md`
>
> 跳过命中时 1a 不产出 `_outline-draft.md`，Phase 1 门禁对该项放行。复用已有「已完成阶段跳过」断点续传机制（`_progress.md`），**不引入新机制**。混合态（先 outline-tingle 正向产大纲、再 bootstrap 逆向补 voice/character/world）通过此短路点串联。

...
| Aspect | Detail |
|--------|--------|
| **Calls** | `qing-novelist`（六维原作画像 — 分析模式）, `ping-critic`（defect-marking 缺陷标记）, `plan-chapter`（mode="adaptation"）, `chapter-review`（mode="adaptation"） |
| **Produces** | `reference/manuscripts/_analysis/{作品名}.md`, `{draft}/_adaptation-tracker.md` |
...
- ✅ Checkable：返回 `{current_chapter, mode: "改编", alignment_level, 原作画像存在性}` 给用户
- ✅ Exhaustive：阶段 0 画像已落盘 + 逐章循环当前章 plan/评审产物在磁盘
...
| `reference/manuscripts/_analysis/{作品名}.md` | 阶段 0 产物 | 🚫 硬阻断——画像不存在禁止改编 |

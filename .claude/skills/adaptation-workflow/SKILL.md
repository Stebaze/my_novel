...
**0.1 路径感知**：
1. 读 `novel/project-config.md`——若存在，确认「创作模式 = 改编」+「原作来源」路径；**若缺失（首次改编请求），按"创作模式=改编"隐式继续**，并向用户询问「原作来源」路径（project-config.md 由阶段 0.5 Session 2 步骤 2.7A 正式落盘——首次改编流在阶段 0 不要求该文件存在）

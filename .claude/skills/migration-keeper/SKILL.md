...
**outline.md v1 特别处理**：`outline.md` frontmatter 缺 `format_version` 字段或值为 `1`（即 v1 形态）→ 报告为 🟡 建议迁移（不阻断）。具体迁移动作见下方「outline.md v1→v2 迁移规则」小节。

### analyze-content

扫描 6 类：章节（命名/字数/编号）/ 角色（数量/格式）/ 世界（分类）/ 草稿（VALID/INCOMPLETE/LEGACY）/ 游离文件（不在标准清单的 .md）/ 追踪文件（thread-map/character-arcs 存在性）→ 结构化清单。

### outline.md v1→v2 迁移规则

outline.md 模板 v2（Commit 1）引入 `## Premise（原始点子）` 段 + 7 个 frontmatter 状态机字段 + L1「不可违背规则」子项化。旧项目 outline.md 仍 v1（无 frontmatter 或仅 `sections` 字段）。此项规则由 check-compat 报告（🟡 建议迁移）、migrate-project 执行。

**检测条件**：`outline.md` frontmatter 缺 `format_version` 字段，或 `format_version` 值 ≠ `2`（缺失或 =1 均判为 v1）。

**迁移动作**（目标形态对齐 `framework/templates/outline.md` v2）：

1. 补 `## Premise（原始点子）` 段（插入到 L1 之前），3 子项填占位 `(待迁移，请重跑 outline-tingle 或手动补全)`：
   - `**原始一句话**`
   - `**灵感来源**`（可空）
   - `**期望读者感受**`
2. 补 7 个 frontmatter 字段：
   - `format_version: 2`
   - `produced_by: ""`（旧文件来源未知）
   - `produced_at: ""`
   - `mode: original`
   ...
   - `pending_confirm: []`
3. `sections` 数组首位补 Premise 项：`heading: "## Premise（原始点子）"`、`skills: [outline-tingle]`、`desc` 同模板
4. L1「不可违背的规则」子项化：保留原有规则条目重新编号为 `1./2./3./...`，并在标题后补注释「3-5 条，每条带编号，便于门禁精确引用」。若原为段落式描述则拆分为编号条目。

**保留**：原有 L1-L5 已填内容、追踪表（角色出场率/关系里程碑/爆点/伏笔等）、附录、已交付章节表等原样保留——只补 v2 新增结构，不动既有内容。
...
4. migrate-structure：
   ...
   ※ outline.md v1→v2 迁移：Case A/B 均自动跑（见上方「outline.md v1→v2 迁移规则」小节）——Case A 由 check-compat 检测后触发，Case B 在「配置/笔记/大纲迁移」子步骤中执行

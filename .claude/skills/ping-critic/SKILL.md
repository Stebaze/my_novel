...
> **2026-06-29 硬 schema 约束**：本 Step 输出**禁止软散文判断**。原 Ch2 auto 评审因执行 LLM 自拟 6 项口语清单（"排比抒情/段末总结句/对称句式…"）替代 SSOT 方法论表，导致叙述者解码（#9）/V3/V6/V7 四项 🔴 全部漏判为"未检出"。以下 2a-0 + 2h 为强制约束，不可跳过。

```
2a-0. 机器预扫描（强制，零 LLM 判断，不可跳过）：
   对章节正文 {DraftDir}/chapters/chapter-{N}.md 跑 Bash grep 取硬计数，
   结果直接填入 2b/2g/2h 判定表的"实测"列。LLM 不得凭印象下计数。
   
   **grep 规则来源**（2026-06-29 SSOT 动态化）：固定项保留硬编码；已注册指纹
   （fp-N）的 grep 速查串从 `framework/guides/ai-risk-mitigation.md` 动态读取——
   按 `{#fingerprint-N}` 锚点 parse 每条「检测规则」段，提取标注"grep 速查"的
   正则串执行。新指纹入库自动反映，本 SKILL 无需手动同步。无 grep 速查串的指纹
   （L3 纯语义判定：fp-10/12/16/19）跳过机器层，交 2h 语义复核清单。
   
   必跑命令（输出次数+行号，逐项贴入判定表）：
   - V3 AI 高频词：对每个词跑 grep -o "词" chapter-N.md | wc -l 统计总出现次数（注意：grep -oc 统计的是含匹配的行数非次数，同一行多次出现只算 1，故用 -o | wc -l）。词表：慢慢/忽然/原来/突然/不禁/仿佛/缓缓/深深/似乎/非常/然后/美丽/壮观
   - V7 否定断言排比："不是X。是Y。"骨架 → grep -nE "不是[^。]*。[^。]*是" 统计行数
   - V6 句法骨架：句首 2-3 词模式，肉眼+grep 辅助
   - 否定式双句排比 {#fingerprint-5}：grep -nE "他没|她没|没问自己|没意识到|没在脑子里|没去想|没想过"
   - 叙述者解码锚词：grep -nE "忽然懂了|忽然明白|忽然意识到|懂了一件事|忽然就懂"
   - 抽象情感标签：grep -nE "斟酌|复杂|难以言喻|说不清"
   - **已注册指纹 grep（动态读取，按 ai-risk-mitigation.md 锚点 parse）**：
     逐条跑「检测规则」段标注的 grep 速查串，计数填入 2h 第四张表实测列。
     含 grep 速查的指纹（fp-8/9/11/13/14/15/17/18/20 等）执行；无 grep 速查的
     （fp-10/12/16/19）跳过——这些是 L3 纯语义判定，grep 无能为力。
   
   → 计数 ≥ 阈值的项目自动标 🔴/🟡，LLM 不得下调至 ✅。
   → 预扫描结果作为 2h 判定表的证据列，未跑预扫描 = Step 2 未完成。
...
2h. 逐项判定表（强制输出，禁止省略/合并）：
   报告 §3 必须含四张完整表 + 一张语义复核清单，每行 ✅/⚠️/❌（或 ✅/🟡/🔴）+ 实测计数/位置证据：
   - 五指纹表（5 行：过度平滑/声音均化/合理偏置/语境衰减/叙述者解码）
   - 15 项自检表（15 行，逐项不合并——禁止"1-5 风格层"式压缩）
   - V1-V10 表（10 行）
   - **已注册指纹检测表**（L1/L2，动态行数——按 ai-risk-mitigation.md 已入库且含 grep
     速查的指纹逐条列：fp-8/9/11/13/14/15/17/18/20 等。实测列填 2a-0 grep 计数，
     每行附指纹名+grep 命中行号+判定。零命中写"0 处（扫描范围：全章正文）"）
   - **语义复核清单**（L3 纯语义判定，4 条——grep 无能为力，靠判定问题）：
     · fp-10 平行结构破坏：逗号/顿号并列的成分语法层级是否对齐？（一侧含动词另一侧纯名词短语→🔴）
     · fp-12 分段错误：相邻段/句是否同一语义单元被强行断开？（宾语细化/补充说明/动作流因果链被句号或段断切→🔴）
     · fp-16 无信息感官填充：动作句中的感官是否推进认知/承载情绪/暗示状态？（三问全否→🔴）
     · fp-19 POV 越界全知断言：POV 角色是否断言另一角色心理动作？（断言主语≠当前 POV 角色 + 内容为心理非物理→🔴）
     每条独立判定 + 证据（位置+引用），零命中写"0 处（扫描范围：全章正文）"
   禁止无依据"未检出"——零命中写"0 处（扫描范围：全章正文）"并附扫描范围。
   候选指纹区恒存在（零候选写 `new_fingerprint_candidates: []`，不省略本区）。
```

**结论闸门（强制）**：写"AI 指纹风险低 / 未检出 / pass"前，必须自检四张表 + 语义复核清单全部有判定 + 证据（五指纹 5 + 15 项 15 + V1-V10 10 + 已注册指纹表动态行 + L3 清单 4，零命中也算"有判定"）。任一行缺判定或缺证据 → **禁止下结论**，返回补齐该行。此闸门是阻断"自拟清单偷工"的硬约束——Ch2 漏检的根因正是结论先于判定写下。L3 语义复核清单与四张表同级受约束，不可跳过。

...
```
2a-0. 机器预扫描（可选——清单核对为主）：
   对章节正文 {DraftDir}/chapters/chapter-{N}.md 按 [`framework/guides/ai-fingerprint-checklist.md`](../../framework/guides/ai-fingerprint-checklist.md)
   10 条检测项逐项核对。重点关注章级累计型（#3 破折号注释：章内 ≥3 触发）。
   LLM 凭印象核对即可——清单是简明的"修法指引"，不需要复杂 grep 协议。
   
   必跑命令（简化版——只保留最常见几条）：
   - V3 AI 高频词：grep -oE "慢慢|忽然|原来|突然|不禁|仿佛|缓缓|深深|似乎|非常|然后|美丽|壮观" chapter-N.md | wc -l
   - 否定式元认知：grep -nE "他没|她没|没问自己|没意识到|没在脑子里|没去想|没想过" chapter-N.md
   - 抽象情感标签：grep -nE "斟酌|复杂|难以言喻|说不清" chapter-N.md
   - 破折号注释章级累计：grep -oE "——" chapter-N.md | wc -l（≥3 触发 #3 判定）
   
   其余条目（#5 平行结构破坏 / #6 分段错位 / #7 视角违反 / #8 心理呈现 / #10 抽象情感标签）
   是 L3 纯语义判定，LLM 人工核对。
   
   → 预扫描结果作为 2h 判定表的证据列，未跑 = Step 2 未完成。
...
2h. 逐项判定表（强制输出，禁止省略/合并）：
   ...
   - V1-V10 表（10 行）
   - **AI 指纹检测表**（10 行——按 [`framework/guides/ai-fingerprint-checklist.md`](../../framework/guides/ai-fingerprint-checklist.md) 10 条逐条列：
     实测列填 2a-0 grep 计数，每行附指纹名+grep 命中行号+判定。
     零命中写"0 处（扫描范围：全章正文）"）
   - **语义复核清单**（4 条——grep 无能为力的纯语义判定）：
     · #5 平行结构破坏：逗号/顿号并列的成分语法层级是否对齐？（一侧含动词另一侧纯名词短语→🔴）
     · #6 分段错误：相邻段/句是否同一语义单元被强行断开？（宾语细化/补充说明/动作流因果链被句号或段断切→🔴）
     · #7 视角违反（含 POV 越界）：叙述者全知注释 / 目光实体化 / POV 越界断言另一角色心理？
     · #8 心理呈现错误：是否「知道」框架穷举已立设定 / 叙述句+独白句号拼接？
...
> **优先级契约（2026-07-01）**：本 Skill 评审时遇到指纹判定与已加载 profile 描述冲突，遵循 `framework/guides/ai-writing-dna.md`「AI 指纹 avoidance 整体 > 任何 base/theme/style/author 描述」——以指纹判定为准。

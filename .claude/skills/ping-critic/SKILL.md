...
2a-0. 机器预扫描（强制，零 LLM 判断，不可跳过）：
   ...
   结果直接填入 2b/2g 判定表的"实测"列。LLM 不得凭印象下计数。
...
2h. 逐项判定表（强制输出，禁止省略/合并）：
   报告 §3 必须含三张完整表，每行 ✅/⚠️/❌（或 ✅/🟡/🔴）+ 实测计数/位置证据：
...
**结论闸门（强制）**：写"AI 指纹风险低 / 未检出 / pass"前，必须自检三张表 30 行（5+15+10）全部有判定 + 证据。任一行缺判定或缺证据 → **禁止下结论**，返回补齐该行。此闸门是阻断"自拟清单偷工"的硬约束——Ch2 漏检的根因正是结论先于判定写下。

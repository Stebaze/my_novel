...
**不做碎片想法捕获**：发散产出的想法由作者决定是否 `/spark` 记录，思源不自动写入灵感日志。记录职责收敛到 `spark` Skill（思源只负责"识别 + 提示"）。
...
2. 等待作者回应：
   ...
   └── "有意思但不是现在" → 提示作者用 `/spark <方向>` 记到灵感日志，继续
...
1. 整理产出为一览表：
   ...
   - 关联的情绪类型和场景类型
   - 与伏笔/弧光/线程的可能关联（如有）
   - 对"不适合本章"方向的标注（说明为什么——提示作者用 `/spark <方向>` 记入灵感日志备未来使用）
...
2. 同时执行：
   - 对"被作者标记为有趣但不适合本章"或"不适合本章但思路有趣"的方向，提示作者用 `/spark <方向>` 记入灵感日志（不自动写入——记录职责收敛到 spark Skill）
...
7. **灵感捕获**：对话中产生的好台词/场景画面/角色反应 → 提示作者"这个值得 `/spark <内容>` 记一下吗？"（不自动写入——记录职责收敛到 spark Skill）

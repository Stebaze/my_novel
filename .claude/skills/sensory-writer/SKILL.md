...
5. **声纹承载原则**（v2.0.1）：角色声纹特征靠「句式/词汇/口头禅/语法结构」承载, **不靠「段数/独立段」承载**。
   - 番长声纹 = 自嘲吐槽+短句+内心活跃+出口 vs 内心鸿沟
   - 实现方式: 句式层面("我"+短句+自嘲语气词), 不在分段层面(不是 1 句 1 段)
   - 当声纹要求 1 句 1 段 vs 读感要求合并时, **默认选合并 + 声纹靠句式保留**。
   - 1 句独立段的合法触发: 新场景/新时间/新 POV/动作结果/重要情绪爆点。
   - 「短段」≠「碎片化」: 短段应承载高密度内容, 不是把同一思维拆成多段。
...
4a. [括号独白改写示范] 当你想写「（我不喜欢她。）」时,自问"能不能改成叙述流":
   ❌ (我不喜欢她。)
     → 注释感重+ 通道单薄
   ✅ 我试着说服自己——不喜欢她。但没用。
     → 自由间接引语+ 番长 voice (自嘲)+ 叙述流连贯
   ✅ 我心里知道,其实并不讨厌她。
     → 第三人称叙述+ 角色腔调

   ❌ (米芙露不会喜欢我的。)
     → 1 句独立段, 通道坍缩
   ✅ 米芙露不会喜欢我——这种自知之明今天特别清晰。
     → 叙述+破折号+番长 voice

   ❌ (完了。)/(她要说了。)/(她要说"你喜欢我"了。)  // 3 短括号瀑布
   ✅ 完了。她要说了——她要说"你喜欢我"了。
     → 1 段,3 句递进, 用句号+破折号分层, 不拆段

   ❌ (我不喜欢她。)/(我不喜欢她。)  // 同主题重复
   ✅ 我试着说不喜欢她。但这话连自己都骗不过。
     → 重复消失, 改为自嘲

   判定信号: 当连续 ≥2 句括号独白时, 至少合并 1 处为叙述流。连续 ≥3 句 → 必须合并 ≥2 处。
   保留 ( ) 的合法触发: 极短独立判断（【三个不够。】/【读得透。但——】）/ 短促自我吐槽/ 强声纹特征（番长式 1 句爆点）/ 段末留白
...
0.5. [动笔前通顺预读] 30 秒扫读 5 维度（不写正文，只过脑）：
   ...
   5. [段间合并预判] 预读每个节拍草拟的段切分,标出"该合并"的段对:
      ...
      [反模式: 同主题重复递进] "米芙露不会.../不喜欢.../米芙露——" 类递进重复
        必须合并为 1 段, 用逗号/分号/破折号连气。合并后内部用"——"分层
        表达递进节奏,但不拆段。

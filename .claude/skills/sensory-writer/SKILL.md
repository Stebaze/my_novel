...
4. **指纹意识分工（2026-06-29 轻量原则）**——生成时(Step 2 逐句)仅覆盖**句式指纹**(7 模式表:否定断言排比/叙述者解码/否定式双句排比等),下笔时识别并规避。**词汇指纹**(慢慢/忽然/原来 等高频词累计)不在生成端判断——单句层无法知章级累计,交 generate-chapter 2b-gate 追踪器 + 章末 chapter-review 统一处理。生成端不加重的计数负担。
...
6. [自由间接引语] 心理是否融在叙述句（第三人称带角色腔调），无句号拼接独白/无「他想」标签？
   → 命中 fp-18 贴标签结构 → 改写为自由间接引语（不删心理，改通道）
```

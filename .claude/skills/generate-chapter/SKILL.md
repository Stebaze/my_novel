...
Round 1（修复）：
  ...
  4. 更新 scene-summaries.json + 重新拼装章节（重拼后**不重算字数**——实测字数统一由 chapter-review Step 3 统计回填；Fix 改了场景文本，字数变化在下一轮 Step 3 评审时自动覆盖）
...
| `framework/guides/ai-writing-dna.md` | Step 2 方法论 | ⚠️ 方法已内置，标注缺失 |
